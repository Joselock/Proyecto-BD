...
public class MainWindowController implements Initializable, TurnosRevisadosListener {
    ...
    private void animarNumero(Label label, int valorFinal, int delayInicio) {
        ...
        // Usar un array de enteros para almacenar el valor actual
        // Esto evita el problema de "effectively final"
        final int[] valorActual = {0};
        ...
        for (int i = 0; i <= pasos; i++) {
            ...
            // Asegurar que el último paso sea exacto
            if (i == pasos) {
                valorEnPaso = valorFinal;
            }
            
            // Guardar el valor en una variable final para el lambda
            ...
            KeyFrame keyFrame = new KeyFrame(
                tiempo,
                event -> {
                    label.setText(String.valueOf(valorMostrar));
                }
            );
            timeline.getKeyFrames().add(keyFrame);
        }
        
        // Asegurar que el valor final se muestre correctamente
